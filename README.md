...
#### 注：白名单模式规则处理逻辑
#### 1. Docker等使用NAT转发的，请在‘拒绝接口’中排除相应接口或添加MAC地址到规则
#### 2. ‘禁网时段’不为空时，则按‘黑名单’规则处理
#### 3. ‘禁网时段’为空和‘星期’为每天（星期1~7）时，则表示全开放
#### 4. ‘禁网时段’为空和‘星期’不为每天（星期1~7）时，则选中‘星期’的所有时段禁网（即：禁网时段为“00:00:00-23:59:59”）
